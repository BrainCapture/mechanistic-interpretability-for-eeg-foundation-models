...
cache required).  Codebook Browser and Morphology Probe fall back to any
matching pre-built app_cache.pt in results/experiments/.
"""
...
# HDF5 metadata fields → display names for Codebook Browser color options
...
def page_home() -> None:
    ...
    cards = [
        ...
        ("Codebook Browser", "Explore the UMAP of token-type clusters. Colour by band, label, patient metadata, or cluster quality.", "App cache"),
        ("Layer Explorer", "Animated UMAP across encoder layers — watch representations evolve with depth.", "Layer UMAP cache"),
        ("Morphology Probe", "Embed 8 hand-crafted clinical waveforms and find their nearest codebook clusters.", "App cache"),
...
| 3 | `build_codebook.py --encoder {ENCODER}` | `results/features/{ENCODER}/codebook_*.pt` | Codebook Browser |
| 4 | `build_app_cache.py --experiment {ENCODER}_layer{L}` | `results/experiments/…/app_cache.pt` | Most pages |
| 5 | `run_tcav.py --experiment {ENCODER}_layer{L}` | `results/tcav/…/tcav_cache.pt` | TCAV Explorer, Concept Steering |
| 6 | `build_layer_umap_cache.py --encoder {ENCODER}` | `results/layer_umap/{ENCODER}/umap_cache.pt` | Layer Explorer |
| 7 | `feature_maximization.py --experiment {ENCODER}_layer{L}` | `results/feat_viz/…/prototypes.npz` | Feature Explorer (prototypes tab) |
...
# Page 2 — Codebook Browser
# ─────────────────────────────────────────────────────────────────────────────

def page_codebook_browser(app_cache: Optional[dict], folder_name: str, layer: int):
    st.title("Codebook Browser")

    if app_cache is None:
        exp_name = f"{folder_name}_layer{layer}"
        st.info("The Codebook Browser requires a pre-built app cache.")
        _build_app_cache_button(exp_name, "_codebook")
        return

    cache = app_cache
    st.markdown(
        "Explore the **token-type codebook** — prototypical 1-second EEG token types "
        "built by clustering validation tokens in whitened spectral amplitude space."
    )

    if "codebook_umap_coords" not in cache:
        st.warning("UMAP coordinates not in cache. Re-run build_app_cache.py "
                   "without --skip-umap.")
        return

    umap_xy       = cache["codebook_umap_coords"]
    umap_labels   = cache["codebook_umap_labels"]
    tok_labels    = cache.get("codebook_umap_token_labels")
    K             = int(cache["codebook_n_clusters"])
    band_labels   = cache["codebook_band_labels"]
    cluster_sizes = cache["codebook_cluster_sizes"]
    if torch.is_tensor(cluster_sizes):
        cluster_sizes = cluster_sizes.numpy()
    recon_r = cache["codebook_exemplar_recon_r"]
    if torch.is_tensor(recon_r):
        recon_r = recon_r.numpy()
    sort_order = cache["codebook_sort_order"]
    if torch.is_tensor(sort_order):
        sort_order = sort_order.numpy()
    recon_waves = cache["codebook_recon_waveforms"]
    amp_log1p   = cache["codebook_amp_log1p"]
    if torch.is_tensor(amp_log1p):
        amp_log1p = amp_log1p.numpy()
    freqs_all   = cache["codebook_freqs"]
    band_powers = cache["codebook_band_powers"]

    FS         = cache.get("meta", {}).get("fs", 128)
    clean_mask = freqs_all <= 45.0
    freqs      = freqs_all[clean_mask]
    band_names = list(BAND_COLORS.keys())

    # HDF5 metadata arrays (token-level, already subsampled to match umap_xy)
    token_meta: dict = cache.get("codebook_umap_meta", {})

    # ── Controls ──────────────────────────────────────────────────────
    base_color_opts = (
        ["Dominant band", "Cluster index", "Waveform quality (r)", "Cluster size"]
        + (["Token label (0/1)"] if tok_labels is not None else [])
    )
    meta_color_opts = [
        META_LABELS[f] for f in META_LABELS if f in token_meta
    ]
    ...
    with col1:
        color_group = st.selectbox(
            "Color by",
            ["— Codebook —"] + base_color_opts
            + (["— Patient metadata —"] + meta_color_opts if meta_color_opts else []),
        )
        # Skip section headers
        if color_group.startswith("—"):
            color_by = base_color_opts[0]
        else:
            color_by = color_group
    with col2:
        filter_band2 = st.selectbox("Highlight band", ["All"] + band_names)
    with col3:
        means_only = st.toggle("Cluster means", value=False)

    # Reverse-map display name → field key for metadata options
    _meta_label_to_field = {v: k for k, v in META_LABELS.items()}

    # ── Compute cluster centroids in UMAP space (for means-only view) ──
    cluster_means_xy = np.array([
        umap_xy[umap_labels == k].mean(axis=0)
        if (umap_labels == k).any() else np.zeros(2)
        for k in range(K)
    ])  # (K, 2)

    def _build_umap_figure(xy, labels_arr, is_means):  # noqa: C901
        cscale = None

        if color_by == "Dominant band":
            colors = [BAND_COLORS.get(band_labels[lbl], "#aaaaaa") for lbl in labels_arr]
        elif color_by == "Cluster index":
            colors = labels_arr.tolist()
            cscale = "Viridis"
        elif color_by == "Waveform quality (r)":
            colors = recon_r[labels_arr].tolist()
            cscale = "RdYlGn"
        elif color_by == "Cluster size":
            colors = np.log1p(cluster_sizes[labels_arr]).tolist()
            cscale = "Blues"
        elif color_by == "Token label (0/1)" and tok_labels is not None:
            colors = tok_labels[labels_arr].tolist()
            cscale = "RdBu"
        elif color_by in _meta_label_to_field:
            field = _meta_label_to_field[color_by]
            raw_vals = token_meta[field]  # (n_tokens,) string array

            # Parse recording_date to year
            if field == "recording_date":
                raw_vals = np.array([
                    v[:4] if len(v) >= 4 else v for v in raw_vals
                ], dtype=object)

            if is_means:
                # Modal value per cluster (most frequent)
                def _mode(arr):
                    if len(arr) == 0:
                        return ""
                ...
                point_vals = np.array([
                    _mode(raw_vals[umap_labels == k]) for k in labels_arr
                ], dtype=object)
            else:
                # raw_vals is already per-token (same length/order as xy)
                point_vals = raw_vals

            unique_vals = sorted(set(v for v in point_vals if v != ""))
            palette = _META_QUAL_PALETTE
            val_to_color = {v: palette[i % len(palette)] for i, v in enumerate(unique_vals)}
            colors = [val_to_color.get(str(v), "#888888") for v in point_vals]
        else:
            colors = labels_arr.tolist()
            cscale = "Viridis"

        # Per-cluster subject diversity (for confound annotation)
        subj_vals = token_meta.get("subject_id")
        if subj_vals is not None:
            n_subj_per_cluster = {
                k: int(np.unique(subj_vals[umap_labels == k]).size)
                for k in range(K)
            }
        else:
            n_subj_per_cluster = {k: 0 for k in range(K)}

        is_meta = color_by in _meta_label_to_field and _meta_label_to_field[color_by] in _META_CONFOUND_CHECK_FIELDS

        if is_means:
            marker_size = (np.sqrt(cluster_sizes) / np.sqrt(cluster_sizes).max() * 20 + 6).tolist()
            opacity_arr = (
                [1.0 if band_labels[k] == filter_band2 else 0.15 for k in labels_arr]
                if filter_band2 != "All" else [0.9] * len(labels_arr)
            )
            hover = [
                f"C{k:03d} | {band_labels[k]} | n={int(cluster_sizes[k])} | r={recon_r[k]:.2f}"
                + (f" | {n_subj_per_cluster[k]} subj" if is_meta else "")
                for k in labels_arr
            ]
        else:
            marker_size = 6
            opacity_arr = (
                np.where(np.array(band_labels)[labels_arr] == filter_band2, 1.0, 0.08).tolist()
                if filter_band2 != "All" else [0.75] * len(labels_arr)
            )
            hover = [
                f"Cluster {lbl}"
                + (f" | {n_subj_per_cluster[lbl]} subj" if is_meta else "")
                for lbl in labels_arr
            ]

        common_marker = dict(size=marker_size, line=dict(width=0))
        if colors and isinstance(colors[0], str):
            common_marker["color"]   = colors
            common_marker["opacity"] = opacity_arr
        else:
            common_marker.update(dict(color=colors, colorscale=cscale,
                                      showscale=True, opacity=opacity_arr))
        ...
        fig.update_layout(
            ...
            height=480, margin=dict(t=10, b=40, l=40, r=10),
            paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
            font=dict(color="#fafafa"),
            xaxis=dict(showgrid=False, zeroline=False, color="#fafafa"),
            yaxis=dict(showgrid=False, zeroline=False, color="#fafafa"),
        )
        return fig

    if means_only:
        fig_umap = _build_umap_figure(
            cluster_means_xy, np.arange(K), is_means=True)
    else:
        fig_umap = _build_umap_figure(umap_xy, umap_labels, is_means=False)

    st.plotly_chart(fig_umap, use_container_width=True)

    # ── Metadata legend + confound check ──────────────────────────────
    if color_by in _meta_label_to_field:
        field = _meta_label_to_field[color_by]
        raw_all = token_meta[field]
        if field == "recording_date":
            raw_all = np.array([v[:4] if len(v) >= 4 else v for v in raw_all], dtype=object)
        unique_vals = sorted(set(v for v in raw_all if v != ""))
        palette = _META_QUAL_PALETTE
        legend_html = " &nbsp; ".join(
            f"<span style='color:{palette[i % len(palette)]};font-weight:bold'>■</span> {v}"
            for i, v in enumerate(unique_vals)
        )
        st.markdown(legend_html, unsafe_allow_html=True)

        # Confound check — only for non-subject fields
        if field in _META_CONFOUND_CHECK_FIELDS and "subject_id" in token_meta:
            subj_vals = token_meta["subject_id"]
            n_subj_arr = np.array([
                int(np.unique(subj_vals[umap_labels == k]).size) for k in range(K)
            ])
            total_subj = len(np.unique(subj_vals))
            frac_arr = n_subj_arr / max(total_subj, 1)

            with st.expander("Subject diversity per cluster (confound check)", expanded=False):
                st.caption(
                    f"Each bar = fraction of the {total_subj} subjects contributing to that cluster. "
                    "Clusters with very few subjects (left) may show metadata patterns driven by "
                    "patient identity rather than EEG content."
                )
                fig_conf = go.Figure(go.Bar(
                    x=list(range(K)),
                    y=frac_arr[np.argsort(frac_arr)].tolist(),
                    marker_color=[
                        "#d32f2f" if f < 0.1 else "#fb8c00" if f < 0.25 else "#388e3c"
                        for f in np.sort(frac_arr)
                    ],
                    hovertext=[
                        f"C{k:03d} · {n_subj_arr[k]} / {total_subj} subjects"
                        for k in np.argsort(frac_arr)
                    ],
                    hoverinfo="text",
                ))
                fig_conf.update_layout(
                    xaxis_title="Clusters (sorted by subject diversity)",
                    yaxis_title="Fraction of subjects",
                    yaxis=dict(range=[0, 1], tickformat=".0%"),
                    height=220, margin=dict(t=10, b=40, l=50, r=10),
                    paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                    font=dict(color="#fafafa"),
                    xaxis=dict(showgrid=False, showticklabels=False),
                    shapes=[dict(type="line", x0=0, x1=K, y0=0.1, y1=0.1,
                                 line=dict(color="#d32f2f", dash="dot", width=1))],
                )
                st.plotly_chart(fig_conf, use_container_width=True)
                n_confounded = int((frac_arr < 0.1).sum())
                if n_confounded:
                    st.warning(
                        f"**{n_confounded} / {K} clusters** have <10% subject coverage — "
                        "metadata patterns in those clusters may reflect patient identity, "
                        "not EEG content. Compare with **Subject** coloring to verify."
                    )
    ...
    with col_c1:
        sort_mode = st.selectbox(
            "Sort clusters by",
            ["Total power (default)", "Dominant band",
             "Waveform quality (r) ↓", "Cluster size ↓"],
        )
        if sort_mode == "Total power (default)":
            display_order = sort_order
        elif sort_mode == "Dominant band":
            display_order = sorted(range(K), key=lambda c: band_labels[c])
        elif sort_mode == "Waveform quality (r) ↓":
            display_order = np.argsort(-recon_r)
        else:
            display_order = np.argsort(-cluster_sizes)

        display_opts = [
            f"C{c:03d} | {band_labels[c]} | n={int(cluster_sizes[c])} | r={recon_r[c]:.2f}"
            for c in display_order
        ]
        chosen_disp = st.selectbox("Select cluster", display_opts)
        chosen_c    = display_order[display_opts.index(chosen_disp)]

    with col_c2:
        t_axis  = np.arange(recon_waves.shape[-1]) / FS
        n_wave_ch = recon_waves.shape[1]
        ch_names_wave = _CHANNEL_NAMES[:n_wave_ch] if n_wave_ch <= len(_CHANNEL_NAMES) else [f"ch{i}" for i in range(n_wave_ch)]

        _wave_spacing = 2.5
        fig_wave = go.Figure()
        for ch_i in range(n_wave_ch):
            sig = recon_waves[chosen_c, ch_i]
            if torch.is_tensor(sig):
                sig = sig.numpy()
            sig = sig.astype(float)
            sig = (sig - sig.mean()) / (sig.std() + 1e-8)
            fig_wave.add_trace(go.Scatter(
                x=t_axis.tolist(), y=(sig + ch_i * _wave_spacing).tolist(), mode="lines",
                name=ch_names_wave[ch_i], line=dict(width=1),
                showlegend=False,
                hovertemplate=f"{ch_names_wave[ch_i]}<extra></extra>",
            ))
        fig_wave.update_layout(
            title=f"Cluster {chosen_c} — reconstructed waveforms (r={recon_r[chosen_c]:.3f})",
            xaxis_title="Time (s)",
            yaxis=dict(
                tickmode="array",
                tickvals=[i * _wave_spacing for i in range(n_wave_ch)],
                ticktext=ch_names_wave,
                tickfont=dict(size=9), showgrid=False,
            ),
            height=520, margin=dict(t=40, b=30, l=45, r=10),
        )
        st.plotly_chart(fig_wave, use_container_width=True)

        amp_lin = np.expm1(amp_log1p[chosen_c, clean_mask])
        fig_spec2 = go.Figure(go.Scatter(
            x=freqs.tolist(), y=amp_lin.tolist(),
            mode="lines", fill="tozeroy",
            line=dict(color=BAND_COLORS.get(band_labels[chosen_c], "#555"), width=2),
        ))
        for bname, (f_lo, f_hi) in CLINICAL_BANDS.items():
            fig_spec2.add_vrect(x0=f_lo, x1=f_hi,
                                fillcolor=BAND_COLORS[bname],
                                opacity=0.07, layer="below", line_width=0)
        fig_spec2.update_layout(
            title="Centroid amplitude spectrum (0.5–45 Hz)",
            xaxis_title="Frequency (Hz)", yaxis_title="Amplitude",
            height=230, margin=dict(t=40, b=30, l=40, r=10), showlegend=False,
        )
        st.plotly_chart(fig_spec2, use_container_width=True)

    with st.expander("Band distribution across all clusters"):
        bp_arr = np.stack(
            [np.asarray(band_powers[b], dtype=float) for b in band_names], axis=1
        )
        fig_heat = px.imshow(
            bp_arr[sort_order].T,
            x=[f"C{sort_order[i]:03d}" for i in range(K)],
            y=band_names,
            color_continuous_scale="YlOrRd",
            aspect="auto",
            labels={"color": "Power"},
            title="Band power per cluster (sorted by total power)",
        )
        fig_heat.update_layout(height=300, margin=dict(t=40, b=20, l=60, r=10))
        st.plotly_chart(fig_heat, use_container_width=True)

    # ── Layer activation UMAPs ────────────────────────────────────────
    layer_umap_layers = cache.get("layer_umap_layers")
    if layer_umap_layers:
        st.markdown("---")
        st.subheader("How the latent space evolves through encoder layers")
        st.caption(
            "Each panel shows UMAP of encoder activations at that layer, "
            "coloured by spectral codebook cluster (same palette as above). "
            "Build with `--layer-umap-layers 0,4,8,…` when running `build_app_cache.py`."
        )

        layer_umap_coords   = cache["layer_umap_coords"]          # dict[int → (N,2)]
        layer_umap_cl_labels = cache["layer_umap_cluster_labels"]  # (N,)

        lul_colors = [BAND_COLORS.get(band_labels[lbl], "#aaaaaa")
                      for lbl in layer_umap_cl_labels]

        # ── Small-multiples comparison ────────────────────────────────
        n_layers = len(layer_umap_layers)
        ...
        fig_grid = make_subplots(
            ...
            subplot_titles=[f"Layer {L}" for L in layer_umap_layers],
            horizontal_spacing=0.05, vertical_spacing=0.12,
        )
        for idx, L in enumerate(layer_umap_layers):
            r, c = divmod(idx, n_cols)
            xy = layer_umap_coords[L]
            fig_grid.add_trace(
                go.Scattergl(
                    x=xy[:, 0].tolist(), y=xy[:, 1].tolist(),
                    mode="markers",
                    marker=dict(color=lul_colors, size=3, opacity=0.6,
                                line=dict(width=0)),
                    hoverinfo="skip",
                    showlegend=False,
                ),
            ...
            fig_grid.update_xaxes(showgrid=False, zeroline=False, showticklabels=False,
                                   row=r + 1, col=c + 1)
            fig_grid.update_yaxes(showgrid=False, zeroline=False, showticklabels=False,
                                   row=r + 1, col=c + 1)
        ...
        # ── Single-layer detail ───────────────────────────────────────
        with st.expander("Explore a single layer (full size)"):
            sel_layer = st.select_slider(
                "Layer", options=layer_umap_layers,
                key="layer_umap_slider",
            )
            xy_sel = layer_umap_coords[sel_layer]
            fig_single = go.Figure(go.Scattergl(
                x=xy_sel[:, 0].tolist(), y=xy_sel[:, 1].tolist(),
                mode="markers",
                marker=dict(color=lul_colors, size=5, opacity=0.7,
                            line=dict(width=0)),
                hovertext=[f"cluster {lbl} · {band_labels[lbl]}"
                           for lbl in layer_umap_cl_labels],
                hoverinfo="text",
            ))
            fig_single.update_layout(
                title=f"Encoder activations — Layer {sel_layer}",
                xaxis_title="UMAP-1", yaxis_title="UMAP-2",
                height=500, margin=dict(t=50, b=40, l=40, r=10),
                paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                font=dict(color="#fafafa"),
                xaxis=dict(showgrid=False, zeroline=False, color="#fafafa"),
                yaxis=dict(showgrid=False, zeroline=False, color="#fafafa"),
...
# ─────────────────────────────────────────────────────────────────────────────
# Page 3 — Morphology Probe
# ─────────────────────────────────────────────────────────────────────────────

def page_morphology_probe(app_cache: Optional[dict], folder_name: str, layer: int):
    st.title("Morphology Probe")

    if app_cache is None:
        exp_name = f"{folder_name}_layer{layer}"
        st.info("The Morphology Probe requires a pre-built app cache.")
        _build_app_cache_button(exp_name, "_morphology")
        return
    ...
    st.markdown(
        "8 synthetic textbook EEG waveforms are embedded through the encoder and "
        "matched to their nearest codebook clusters.  "
        "*Does the embedding space distinguish epileptiform from normal morphology?*"
    )

    if "morph_names" not in cache:
        st.warning("Morphology probe data not in cache. Re-run build_app_cache.py "
                   "without --skip-morphology.")
        return

    names           = cache["morph_names"]
    waveforms       = cache["morph_waveforms"]
    match_ids       = cache["morph_match_cluster_ids"]
    match_dists     = cache["morph_match_dists"]
    match_cos       = cache["morph_match_cosines"]
    match_waves     = cache["morph_match_waveforms"]
    match_raw       = cache.get("morph_match_raw_patches")  # (8, 5, C, P) or None
    match_sizes     = cache["morph_match_sizes"]
    match_bands     = cache["morph_match_dom_bands"]
    inter_dist      = cache["morph_inter_dist"]

    t_axis = np.arange(waveforms.shape[-1]) / FS
    CZ = min(9, waveforms.shape[1] - 1)

    st.subheader("Inter-morphology embedding distances")
    fig_heat = px.imshow(
        inter_dist, x=names, y=names,
    ...
    st.plotly_chart(fig_heat, use_container_width=True)
    st.markdown(
        "_Smaller distance = more similar embeddings.  "
        "Morphologically related patterns cluster together._"
    )
    st.markdown("---")

    st.subheader("Morphology detail")
    chosen_morph = st.selectbox("Select morphology", names)
    mi = names.index(chosen_morph)

    col_m1, col_m2 = st.columns([1, 1])
    with col_m1:
        st.markdown(f"**Synthetic waveform — {chosen_morph} (Cz)**")
        fig_synth = go.Figure(go.Scatter(
            x=t_axis, y=waveforms[mi, CZ].tolist(),
    ...
    with col_m2:
        st.markdown("**Top-5 nearest codebook clusters**")
        for rank in range(min(5, len(match_ids[mi]))):
            c_id  = int(match_ids[mi][rank])
            dist  = float(match_dists[mi][rank])
            cos   = float(match_cos[mi][rank])
            size  = int(match_sizes[mi][rank])
            band  = match_bands[mi][rank]
            bc    = BAND_COLORS.get(band, "#555")
            st.markdown(
                f"**#{rank+1}** — C{c_id:03d} &nbsp; "
                f"<span style='color:{bc}'>{band}</span> &nbsp; "
                f"n={size} &nbsp; dist={dist:.1f} &nbsp; cos={cos:.3f}",
                unsafe_allow_html=True,
            )

    tab_recon, tab_raw = st.tabs(["Cluster reconstructions — Cz", "Raw exemplar — all channels"])

    match_waves_np = (match_waves.numpy() if torch.is_tensor(match_waves)
                      else match_waves)
    colors_match = ["#1565C0", "#1976D2", "#1E88E5", "#42A5F5", "#90CAF9"]
    ...
    with tab_recon:
        ...
        for rank in range(min(5, match_waves_np.shape[1])):
            sig = match_waves_np[mi, rank, CZ]
            fig_matches.add_trace(go.Scatter(
                x=t_axis, y=sig.tolist(), mode="lines",
                name=f"#{rank+1} C{int(match_ids[mi][rank]):03d} ({match_bands[mi][rank]})",
                line=dict(color=colors_match[rank], width=1.5), opacity=0.85,
            ))
    ...
    with tab_raw:
        if match_raw is None:
            st.info("Raw exemplar patches not in cache. Re-run `build_codebook.py` "
                    "and `build_app_cache.py` to add them.")
        else:
            match_raw_np = match_raw.numpy() if torch.is_tensor(match_raw) else np.array(match_raw)
            rank_sel = st.radio(
                "Cluster rank",
                options=list(range(min(5, match_raw_np.shape[1]))),
                format_func=lambda r: f"#{r+1} C{int(match_ids[mi][r]):03d} ({match_bands[mi][r]})",
                horizontal=True,
                key=f"morph_rank_{mi}",
            )
            raw_patch = match_raw_np[mi, rank_sel]   # (C, P)
            n_ch = raw_patch.shape[0]
            ch_labels = _CHANNEL_NAMES[:n_ch]
            spacing = float(np.abs(raw_patch).max()) * 2.5 + 1e-6
            t_patch = np.arange(raw_patch.shape[-1]) / FS
            fig_raw = go.Figure()
            for c in range(n_ch):
                fig_raw.add_trace(go.Scatter(
                    x=t_patch.tolist(),
                    y=(raw_patch[c] + c * spacing).tolist(),
                    mode="lines",
                    line=dict(width=0.9, color=colors_match[rank_sel]),
                    name=ch_labels[c], showlegend=False,
                    hovertemplate=f"<b>{ch_labels[c]}</b><br>t=%{{x:.3f}} s<extra></extra>",
                ))
            fig_raw.update_layout(
                ...
                yaxis=dict(
                    tickmode="array",
                    tickvals=[c * spacing for c in range(n_ch)],
                    ticktext=ch_labels,
                    tickfont=dict(size=9),
                ),
                height=max(400, n_ch * 18),
                margin=dict(l=55, r=10, t=10, b=40),
            )
            st.plotly_chart(fig_raw, use_container_width=True)

    st.markdown("---")
    with st.expander("All morphologies — nearest cluster summary"):
        import pandas as pd
        rows = []
        for i, name in enumerate(names):
            for rank in range(min(5, len(match_ids[i]))):
                rows.append({
                    "Morphology": name,
                    "Rank":    rank + 1,
                    "Cluster": int(match_ids[i][rank]),
                    "Distance": round(float(match_dists[i][rank]), 2),
                    "Cosine":  round(float(match_cos[i][rank]), 3),
                    "Band":    match_bands[i][rank],
                    "Size":    int(match_sizes[i][rank]),
                })
        st.dataframe(pd.DataFrame(rows), use_container_width=True, height=400)


# ─────────────────────────────────────────────────────────────────────────────
# Page 4 — TCAV Explorer
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    if page == "Codebook Browser":
        app_cache = load_app_cache(str(p)) if (p := _app_cache_path(folder_name, layer)) else None
        page_codebook_browser(app_cache, folder_name, layer)
        return
    ...
    if page == "Morphology Probe":
        app_cache = load_app_cache(str(p)) if (p := _app_cache_path(folder_name, layer)) else None
        page_morphology_probe(app_cache, folder_name, layer)
        return
