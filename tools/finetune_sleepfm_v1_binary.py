"""Fine-tune the v1 SleepFM SetTransformer on D4-v3-preprocessed-v2 (128 Hz)
with the binary normal/abnormal label, using the project's canonical splits.

Reproduces the role of `checkpoints/finetuned/sleepfm1.ckpt` but with
verifiable subject-level splits (`checkpoints/finetuned/sleepfm/splits.json`)
so the resulting checkpoint is leakage-free with respect to the SAE / probe
evaluation pipeline.

Outputs
-------
  checkpoints/finetuned/sleepfm_v1_local/finetuned.ckpt    — best val-AUROC ckpt
                                                              (encoder.<...> +
                                                              head.weight/bias)
  checkpoints/finetuned/sleepfm_v1_local/training_log.json
  checkpoints/finetuned/sleepfm_v1_local/metrics.json

Usage
-----
  uv run tools/finetune_sleepfm_v1_binary.py
  uv run tools/finetune_sleepfm_v1_binary.py --epochs 12 --freeze-epochs 2 \
      --batch-size 32 --full-lr 1e-4
"""
...
V3_DATA  = ROOT / "data" / "D4-v3-preprocessed-v2"
PRETRAIN = ROOT / "checkpoints" / "pretrained" / "sleepfm_weights.pt"
SPLITS   = ROOT / "checkpoints" / "finetuned" / "sleepfm" / "splits.json"
OUT_DIR  = ROOT / "checkpoints" / "finetuned" / "sleepfm_v1_local"
...
class SleepFMBinary(nn.Module):
    """SleepFM v1 SetTransformer + scalar logit head."""

    def __init__(self, encoder_module: nn.Module, embed_dim: int = EMBED_DIM):
        super().__init__()
        self.encoder = encoder_module        # SetTransformer
        self.head = nn.Linear(embed_dim, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        pooled, _ = self.encoder(x)          # (B, E), _
        return self.head(pooled).squeeze(-1) # (B,)


def build_model() -> SleepFMBinary:
    backend = load_encoder("sleepfm", weights_path=PRETRAIN)
    model = SleepFMBinary(backend.model, embed_dim=EMBED_DIM)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"  [ok] SleepFM v1 + linear head: {n_params/1e6:.2f}M params")
    return model
...
# Train / eval
# ─────────────────────────────────────────────────────────────────────────────
def _set_encoder_grad(model: SleepFMBinary, requires_grad: bool):
